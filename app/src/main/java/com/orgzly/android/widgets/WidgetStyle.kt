...
object WidgetStyle {
    ...
    /**
     * Colors the trailing "done" checkmark by task state: theme todo color while open,
     * done color once completed. Runtime-only (no layout change).
     */
    @JvmStatic
    fun tintDoneIcon(context: Context, remoteViews: RemoteViews, done: Boolean) {
        val colorScheme = AppPreferences.widgetColorScheme(context)
        val dayNight = context.getString(R.string.day_night)

        val stateColorsKey = if (colorScheme == "dynamic") "dynamic-$dayNight" else colorScheme
        val name = if (done) "done" else "todo"

        val color = getColor(context, stateColorsKey, name)
        remoteViews.setInt(R.id.item_list_widget_done, "setColorFilter", color)
    }
