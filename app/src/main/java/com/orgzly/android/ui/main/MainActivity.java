...
public class MainActivity extends CommonActivity
    ...
    private void setupDisplay(Bundle savedInstanceState) {
        if (savedInstanceState == null) { // Not a configuration change.
            ...
            if (bookId > 0 || queryString != null) {
                DisplayManager.displayBooks(getSupportFragmentManager(), false);
            } else {
                /* Default landing page: today's overview. */
                DisplayManager.displayToday(getSupportFragmentManager(), false);
            }
    ...
    protected void onResume() {
        ...
        if (AppPreferences.syncOnAppOpen(this)) {
            /* User-facing "sync on open" switch - full sync once, independent of auto-sync. */
    ...
    private void prewarmBooksClock() {
        App.EXECUTORS.diskIO().execute(() -> {
            try {
                for (com.orgzly.android.db.entity.BookView book : dataRepository.getBooks()) {
                    dataRepository.prewarmClockForBook(book.getBook().getId());
                }
            } catch (Exception e) {
                if (BuildConfig.LOG_DEBUG) LogUtils.d(TAG, "prewarm failed", e);
            }
        });
    }
    ...
    protected void onResumeFragments() {
        ...
        bm.registerReceiver(receiver, new IntentFilter(AppIntent.ACTION_OPEN_TODAY));
    ...
    private class LocalBroadcastReceiver extends BroadcastReceiver {
        ...
        private void handleIntent(@NonNull Intent intent, @NonNull String action) {
            switch (action) {
                ...
                case AppIntent.ACTION_OPEN_TODAY: {
                    DisplayManager.displayToday(getSupportFragmentManager(), true);
                    break;
                }
