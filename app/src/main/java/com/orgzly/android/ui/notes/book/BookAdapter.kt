    ...
    override fun onBindViewHolder(h: RecyclerView.ViewHolder, position: Int) {
        when {
            ...
            h.itemViewType == HIDDEN_ITEM_TYPE -> {
                ...
                noteItemViewBinder.applyOutlineTint(holder, note, getSelection().contains(note.id))
            }
        }
    }

    /** Updates outline tints; returns true when they changed (caller decides when to repaint). */
    fun setOutlineTints(tints: Map<Long, NoteItemViewBinder.OutlineTint>): Boolean {
        if (tints == noteItemViewBinder.outlineTints) {
            return false
        }
        noteItemViewBinder.outlineTints = tints
        return true
    }
