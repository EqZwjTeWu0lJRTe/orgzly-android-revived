...
import com.orgzly.android.ui.notes.today.TodayFragment;
...
public class DisplayManager {
    ...
    /**
     * Show the "today" overview fragment.
     */
    public static void displayToday(FragmentManager fragmentManager, boolean addToBackStack) {
        if (isFragmentDisplayed(fragmentManager, TodayFragment.FRAGMENT_TAG) != null) {
            return;
        }

        Fragment fragment = TodayFragment.Companion.getInstance();

        replaceFragment(
                fragmentManager,
                R.id.single_pane_container,
                fragment,
                TodayFragment.FRAGMENT_TAG,
                addToBackStack);
    }
