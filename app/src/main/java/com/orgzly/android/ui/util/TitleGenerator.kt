...
class TitleGenerator(
    ...
    fun generateTitle(noteView: NoteView): CharSequence {
        ...
        /* Visually weaken progress cookies ([0%], [2/3], ...) so they don't compete with the
         * heading title itself. */
        weakenStatsCookies(builder)

        return builder
    }

    /** Progress cookies in a heading (e.g. "[50%]", "[1/2]") are de-emphasized: lighter gray,
     * smaller and not bold. */
    private fun weakenStatsCookies(builder: SpannableStringBuilder) {
        val text = builder.toString()

        for (match in STATS_COOKIE_REGEX.findAll(text)) {
            val start = match.range.first
            val end = match.range.last + 1

            builder.setSpan(attributes.postTitleTextColor, start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
            builder.setSpan(attributes.postTitleTextSize, start, end, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE)
    ...
    companion object {
        ...
        /* Org statistics cookies: "[50%]", "[100%]", "[%]", "[1/2]", "[2/2]", "[/]". */
        private val STATS_COOKIE_REGEX = Regex("\\[(?:\\d*%|\\d+/\\d*)]")
    }
}
